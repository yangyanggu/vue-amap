...
class CircleSweepPass {
  ...
  constructor(renderer: any, scene: Scene, camera, options: CircleOptions) {
    ...
    this.depthTarget = new WebGLRenderTarget(size.x, size.y);
  ...
  calCameraVectors() {
    ...
    const forwardDir = (new Vector3(0, 0, 1).applyMatrix4(this.camera.matrixWorld.clone()))
    forwardDir.sub(this.camera.position.clone())
    forwardDir.normalize()

    const rightDir = (new Vector3(-1, 0, 0).applyMatrix4(this.camera.matrixWorld.clone()))
    rightDir.sub(this.camera.position.clone())
    rightDir.normalize()

    const topDir = (new Vector3(0, -1, 0).applyMatrix4(this.camera.matrixWorld.clone()))
    topDir.sub(this.camera.position.clone())
    topDir.normalize()
